...
class GoalRepository:
    ...
    async def create(self, user_id: str, target_month: str, target_kg: float, description: str | None) -> Goal:
        goal = Goal(
            user_id=user_id,
    ...
    async def get_by_user(self, user_id: str) -> list[Goal]:
        result = await self.session.execute(
            select(Goal).where(Goal.user_id == user_id).order_by(Goal.target_month.desc())
        )
