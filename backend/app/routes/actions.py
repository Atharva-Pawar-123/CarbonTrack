...
from app.schemas.action import LogActionRequest, ActionLogResponse, ActionSummaryResponse
...
async def log_action(
    ...
    # Check duplicate
    is_dup = await repo.check_duplicate(current_user.id, request.action_id, request.logged_date)
    if is_dup:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Action already logged for this date")
