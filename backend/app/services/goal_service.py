...
class GoalService:
    async def create_goal(
        self, user_id: str, request: CreateGoalRequest, db: AsyncSession
    ) -> Goal:
        ...
        asyncio.create_task(
            self._generate_plan_bg(
                ai_service,
                str(goal.id),
                str(current_entry.id) if current_entry else None,
            )
        )
    ...
    async def get_user_goals(self, user_id: str, db: AsyncSession) -> list[Goal]:
        ...
        for goal in goals:
            ...
            if entry:
                ...
                if goal.target_kg > 0:
                    goal.progress_pct = max(
                        0.0, min(100.0, (entry.total_kg / goal.target_kg) * 100)
                    )
                else:
    ...
    async def update_goal_progress(
        self, user_id: str, target_month: str, current_total_kg: float, db: AsyncSession
    ):
        repo = GoalRepository(db)
        goals = await repo.get_by_user(user_id)
    ...
    async def _generate_plan_bg(
        self, ai_service: AIInsightService, goal_id: str, entry_id: str | None
    ):
